...
time <- system.time(forest_fit <- staplr_base_forest(incomplete_data, view_index, 100))
...
results$preds <- predict(forest_fit, complete_data$xtest)
results$coefs <- coef(forest_fit)$`Level 2`[[1]][-1]
results$ytest <- complete_data$ytest 
results$ptest <- complete_data$ptest
results$time <- time

save(results, file=paste0("/ ... /forest/", TID, ".RData"))
warnings()
