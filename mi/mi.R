...
# Train a model using mean imputation at the base level
time <- system.time(mean_fit <- staplr_base_mean(incomplete_data, view_index))
...
results$preds <- predict(mean_fit, complete_data$xtest)
results$coefs <- coef(mean_fit)$`Level 2`[[1]][-1]
results$ytest <- complete_data$ytest 
results$ptest <- complete_data$ptest
results$time <- time


save(results, file=paste0("/ ... /mi/", TID, ".RData"))
warnings()
