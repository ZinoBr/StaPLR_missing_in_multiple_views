...
# Fit a model on the complete data
time <- system.time(complete_fit <- MVS(complete_data$xtrain, complete_data$ytrain, views=as.matrix(view_index)))
...
results$preds <- predict(complete_fit, complete_data$xtest)
results$coefs <- coef(complete_fit)$`Level 2`[[1]][-1]
results$ytest <- complete_data$ytest 
results$ptest <- complete_data$ptest
results$time <- time

save(results, file=paste0("/ ... /CDA/", TID, ".RData"))
warnings()
