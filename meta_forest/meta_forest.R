...
time <- system.time(meta_forest_fit <- staplr_meta_forest(incomplete_data, view_index, 100))
...
results$preds <- predict(meta_forest_fit, complete_data$xtest)
results$coefs <- coef(meta_forest_fit)$`Level 2`[[1]][-1]
results$ytest <- complete_data$ytest 
results$ptest <- complete_data$ptest
results$time <- time

save(results, file=paste0("/ ... /meta_forest/", TID, ".RData"))
warnings()
